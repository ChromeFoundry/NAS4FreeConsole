...
{
    /// <summary>
    /// ListViewEx is an extended ListView which can handle embedded controls.
    /// </summary>
    public class ListViewEx : ListView
    {
        ...
        public void AddEmbeddedControl(Control c, int col, int row)
        {
            ...
            {
                this.BeginInvoke(new AddEmbeddedControlDelegate(AddEmbeddedControl), c, col, row, DockStyle.Fill);
                return;
            }
            else
            {
        ...
        public void AddEmbeddedControl(Control c, int col, int row, DockStyle dock)
        {
            if (this.InvokeRequired)
            {
                this.BeginInvoke(new AddEmbeddedControlDelegate(AddEmbeddedControl), c, col, row, dock);
                return;
            }
            else
            {
        ...
        /// <summary>
        /// Thread-Safe delegate for adding controls to a cell.
        /// </summary>
        delegate void AddEmbeddedControlDelegate(Control c, int col, int row, DockStyle dock);
        ...
        public void RemoveEmbeddedControl(Control c)
        {
            if (this.InvokeRequired)
            {
                this.BeginInvoke(new RemoveEmbeddedControlDelegate(RemoveEmbeddedControl), c);
                return;
            }
            else
            {
        ...
        /// <summary>
        /// Thread-Safe delegate for removing a control from a cell.
        /// </summary>
        /// <param name="c"></param>
        delegate void RemoveEmbeddedControlDelegate(Control c);

        /// <summary>
        /// Delete all embedded controls from the embedded control arraylist.
        /// </summary>
        public void RemoveAllEmbeddedControls()
        {
            while (_embeddedControls.Count > 0)
            {
                RemoveEmbeddedControl(((EmbeddedControl)_embeddedControls[0]).Control);
            }
        }
        ...
        /// <summary>
        /// Thread-Safe method to add a ListViewItem to the collection.
        /// </summary>
        /// <param name="lv">A ListViewItem</param>
        public void AddListViewItem(ListViewItem lv)
        {
            if (this.InvokeRequired)
            {
                this.BeginInvoke(new AddListViewItemDelegate(AddListViewItem), lv);
                return;
            }
            else
            {
        ...
        /// <summary>
        /// Thread-Safe delegate for adding a ListViewItem to the ListViewEx Item collection.
        /// </summary>
        delegate void AddListViewItemDelegate(ListViewItem lv);
